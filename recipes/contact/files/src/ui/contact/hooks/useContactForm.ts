...
export function useContactForm() {
  ...
  const form = useForm<Contact.FormValues>({
    ...
    // Lazy: silent while typing, validates on blur or submit. Once a field is
    // errored, re-validates on change so success appears the instant it's fixed.
    mode: "onBlur",
    reValidateMode: "onChange",
    // Errors render inline below each field with role="alert"; default scroll-to-error
    // is just an unwanted jump for sighted users.
